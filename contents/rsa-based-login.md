# RSA-key based login

APPLICATION: Log into an SSH session using an RSA key instead of a regular password for more security and/or privacy (or even convenience).

1. Open the authorized_keys file for editing on the server side:

    ```
    $ vim /home/username/.ssh/authorized_keys
    ```

3. Populate it with the *public* RSA-keys you wish to grand system access (replace the following with your keys):

    ```shell
    <ssh-rsa  user@host>
    ```

5. Instruct your ssh client(s), e.g. Putty on Windows, to use the corresponding _private_ key(s) when connecting to the container.

6. Connect to the ssh server using the account of the user where the authorized_keys file was edited for (usually this is not root; you can `su` to super user once logged in as the regular user).
